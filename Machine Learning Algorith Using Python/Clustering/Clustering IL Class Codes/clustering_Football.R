...
football <- read.csv("C:\\Users\\User10\\Desktop\\Jigsaw\\Cluster analysis\\Song_Football.csv")
str(football)
summary(football)

song <- football[which(football$First_Name=="Song"),] # information abbout song
same <- football[258:268,] # no. of rows near to song
...
football.na <- na.omit(football.c) #DROPPING MISSING VALUES
...
#HOW?
football.na$Tackles[1]
(football.na$Tackles[1]-mean(football.na$Tackles))/sd(football.na$Tackles)
...
library(NbClust)
set.seed(1234)
noculs <- NbClust(football.new,min.nc=2,max.nc=10,method="kmeans")
