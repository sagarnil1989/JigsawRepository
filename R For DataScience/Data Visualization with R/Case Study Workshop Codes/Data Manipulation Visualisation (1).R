...
dat1<-read.csv(file = "/Users/ankit/Documents/Jigsaw/Data manipulation and visualization/audit.csv")
dat1 <- read.csv("~/Documents/Jigsaw/Data manipulation and visualization/Topic 6 - Data manipulation and Visualization/audit.csv")

head(dat1)

#Get all rows where gender is Male and employment is private#
dat2<-filter(dat1, Employment=="Private")
dat2<-filter(dat1, Marital=="Married")
...
dat1%>%group_by(Gender)%>%summarise(MeanIncome=mean(Income), minIncome = min(Income), maxIncome =  max(Income))->summ2



dat1%>%group_by(Gender,Marital)%>%summarise(MeanIncome=mean(Income))->summ2




dat1%>%group_by(Occupation)%>%summarise(MeanIncome=mean(Income))->summ2
...
dat1%>%group_by(Marital,Gender)%>%summarise(MeanIncome=mean(Income))->summ2
...
dat1%>%group_by(Marital)%>%summarise(counted=n(), suminc= sum(Income), mininc = min(Income))->summ3
...
dat1%>%group_by(Gender,Marital)%>%summarise(MeanIncome=mean(Income),counted = n())
...
dat1%>%select(ID,Gender,Income)%>%group_by(Gender)%>%filter(min_rank(desc(Income))<=14)-> summ4#notice how arrange() works here
...
dat1%>%select(ID,Gender,Income)%>%group_by(Gender)%>%filter(cume_dist(desc(Income))<=0.01)%>%arrange(desc(Income))


dat1%>%mutate(income_2 = Income/100)->test
dat1%>%mutate(Group=ntile(desc(Income),10))-> test
...
dat5 <- read.csv("~/Documents/Jigsaw/Data manipulation and visualization/Topic 6 - Data manipulation and Visualization/Strings.csv")
...
p+geom_point(aes(size=Gender,color=Income))+scale_size_discrete(range = c(1,2)) +scale_color_continuous(low="blue",high="red")
...
p+geom_point(aes(size=Gender,color=Income))+scale_size_discrete(labels=c("F","M"),range = c(1,3))+scale_color_continuous(low="blue",high="red")+guides(size=F)+facet_grid(Gender~Employment)#Anything else we can do?
