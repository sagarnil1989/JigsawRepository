...
filter_countries<-ac%>%filter(Country==c("India","China","Germany","Russia","Brazil")) 
...
k2<-p %>% ggplot(aes(x = year_bucket, y = no_fatalities,fill=count_crashes)) + #Y is # people aboard                     
  geom_bar(stat = "identity")+                                      #bar chart
