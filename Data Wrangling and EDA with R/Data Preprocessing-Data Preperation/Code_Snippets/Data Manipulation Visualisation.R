...
dat1<-read.csv("C:\\Users\\suhani\\Documents\\Jigsaw\\Data manipulation and visualization\\Topic 6 - Data manipulation and Visualization\\audit.csv")
head(dat1)
...
dat1%>%select(ID,Gender,Income)%>%group_by()%>%filter(min_rank(desc(Income))<=14)%>%arrange(desc(Income))#notice how arrange() works here
...
dat1%>%select(ID,Marital,Income)%>%group_by(Marital)%>%filter(cume_dist(desc(Income))<=0.01)%>%arrange(desc(Income))
...
dat5<-read.csv("C:\\Users\\suhani\\Documents\\Jigsaw\\Data manipulation and visualization\\Topic 6 - Data manipulation and Visualization\\Strings.csv")
str(dat5)
...
p+geom_point(aes(size=Gender,color=Income))+scale_size_discrete(labels=c("F","M"),range = c(1,3))+scale_color_continuous(low="blue",high="red")+guides(size=F)+facet_grid(Marital~Gender)#Anything else we can do?
